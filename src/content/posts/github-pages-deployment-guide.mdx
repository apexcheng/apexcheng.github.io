...
这个博客属于项目站点，因为仓库名是 `blog`，所以访问路径不是根目录，而是 `/blog/`。
...
Astro 里有两个部署到 GitHub Pages 时很关键的配置：`site` 和 `base`。
...
`site` 表示站点域名，`base` 表示项目站点挂载的子路径。因为这个仓库叫 `blog`，所以 GitHub Pages 的项目站点路径就是 `/blog/`。

<Callout title="为什么 base 很重要？">
如果不配置 `base: '/blog'`，Astro 可能会把资源路径当成根目录 `/`。部署到项目站点后，CSS、JS、文章链接就可能出现 404。
</Callout>
...
GitHub Pages 的项目站点和普通根站点不一样。

如果仓库名是 `blog`，项目站点默认路径就是：

```text
/blog/
```

Astro 默认会认为站点部署在根路径 `/`。但当前博客实际部署在 `/blog/`，所以需要：

```js
base: '/blog'
```

这样构建出来的 CSS、JS、文章链接、RSS 链接等资源，才会带上正确的 `/blog` 前缀。

本项目里还封装了一个 `withBase()` 方法，用来生成带 base 的内部链接：
...
<Callout title="项目改名时要注意">
如果远端仓库名从 `blog` 改成别的名字，除了 GitHub 仓库地址要变，`astro.config.mjs` 里的 `base` 也要同步改。否则线上资源路径容易错。
</Callout>
...
由于项目有 `base: '/blog'`，线上完整访问路径会挂在 `/blog/` 下。

## 常见问题

### 页面能打开，但样式丢了

优先检查 `base` 是否正确。如果仓库名是 `blog`，就应该是：
