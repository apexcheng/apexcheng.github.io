...
| `src/content/posts/excel-functions-for-operations-beginners.mdx` | `教程` | 面向运营新人的 Excel 常用函数清单，按基础和中级两级整理统计、查找、文本、日期、动态数组和批量计算函数，并通过周报、异常清单、SKU 汇总和库存周转讲解综合应用 | 写 Excel / WPS 函数教学、运营新人培训、表格数据处理和动态报表文章时参考 |
