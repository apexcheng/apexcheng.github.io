...
核心原则：**视觉优先、按需读取、目标完整、边界清晰、规则单一来源。**
...
2. 只处理当前任务直接相关的文件；允许为完成当前目标做必要重构，但不做无关重构、格式化或改名。
...
- `design-taste-frontend`：新页面、新视觉概念，或现有页面需要重新探索明显不同的视觉方向时使用；项目现有 Astro / MDX 技术栈、组件和视觉规则优先于 Skill 的通用建议。
- `redesign-existing-projects`：已有文章或页面的视觉审查、重设计默认优先使用；只有需要重新探索新的视觉方向时，再补充 `design-taste-frontend`。普通内容修改不触发。
- `image-to-code`：仅在用户提供截图 / 设计稿 / 明确视觉参考并要求实现或复现，或明确要求 image-first 工作流时使用；不能因为文章属于高视觉文章或会话中出现图片就自动触发。
- `ui-ux-pro-max`：明确涉及无障碍、响应式、图表、配色、字体等专项查询时可以直接使用；普通视觉设计不使用，项目规则和 Taste 已能回答的问题也不重复查询。
- `archify`：复杂系统架构、Agent / Tool 调用链、数据流、生命周期或复杂时序图优先使用；简单关系仍优先 Mermaid。用于正式文章时优先导出 SVG 等静态资源纳入仓库，不把 Archify 作为网站运行时依赖。
